...
export class ProblemListCompactComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    this.problemService.getPage(0, this.pageSize, undefined, undefined, undefined,
      this.status, undefined, this.assignedUser, this.internetUser).subscribe(data => {
      console.log(changes);
...
export class ProblemsDataSource extends DataSource<any> {
  ...
  loadPage(page: number,
    pageSize: number, assignedUser?: string | undefined, status?: string | undefined, internetUser?: number | undefined) {
    this.reportService.getPage(page,
      pageSize, undefined, undefined, undefined, status, undefined, assignedUser, internetUser).subscribe(data => {
